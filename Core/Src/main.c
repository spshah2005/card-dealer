...
//Logic to Hardware Functions
void Deal_Card(int *Cards){ // this call deals the cards
	static int card = 0;
	static int num_of_cards = 0;
	HAL_Delay(500); // wait for turn to be complete
	__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, 1249); // turn the sevro on to have card read from RFID
	HAL_Delay(500); // wait for card to move
...
//Quinn's functions
void Turn_To_Person(int player, int rpm){ // this call finds the right person to get the card to
	static int current_player = 0;
	static int facing = 0;
	int diff = player - current_player;
	if(diff > 0){
		stepperOffsetAngle((360 / num_player) * diff, 1, rpm);
		facing += (360 / num_player) * diff;
	}
	else if(diff < 0){
		offsetstepperOffsetAngleAngle((-360 / num_player) * diff, 0, rpm);
		facing -= (-360 / num_player) * diff;
	}
	current_player = player;
}

void Three_Card_Poker(int *Cards){ // kinda the main
	int rpm = 750;
	for(int i = 0; i < 2; ++i){
		for(int j = 0; j < num_player; j++){ // double check num_player input
			Turn_To_Person(j, rpm);
			Deal_Card(Cards);
		} // end of loop j
		// deal to the Dealer bc we cant go in a circle forever
		Turn_To_Person(0, rpm);
		Deal_Card(Cards);
	} // end of loop i
}

//end of hardware dealing functions
/* USER CODE END 0 */
...
//__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, 1000); // Toggles Servo speed ( =1499 : stop;  <1499 : clockwise; >1499 : counter-clockwise)
...
//HAL_GPIO_WritePin(GPIOE, GPIO_PIN_15, 1);
  ...
  while (1)
  {
	  __HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_3, 1299); // togle sero speed, 1499 = stop, 1499 < cw; 1499 > ccw
//	  int card_no = MFRC522_ReadCard();
//	  if (card_no != -1) {
//		  printf("card_no: %d\r\n", card_no);
//	  }
//
	  HAL_Delay(1000);
	  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_15, 1); // 1 turns on dc motor, 0 if off
	  HAL_Delay(1000);
	  HAL_GPIO_WritePin(GPIOE, GPIO_PIN_15, 0);
//	  HAL_Delay(1000);
    /* USER CODE END WHILE */
...
//	  HAL_Delay(10000);
//	  for (int i = 0; i < 16; ++i) {
//		  stepperOffsetAngle(22.5, 0, 500);
//		  HAL_Delay(500);
//	  }
//	  for (int i = 0; i < 8; ++i) {
//		  stepperOffsetAngle(45, 1, 100);
//		  HAL_Delay(1000);
//	  }
//	  for (int i = 0; i < 8; ++i) {
//		  stepperOffsetAngle(180, 0, 750);
//		  HAL_Delay(1000);
//	  }
	  PS2_Update();
	  if (PS2.UP) {
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pins : PS2_SCK_Pin PS2_MOSI_Pin PE15 */
  GPIO_InitStruct.Pin = PS2_SCK_Pin|PS2_MOSI_Pin|GPIO_PIN_15;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
