...
int logic(int num_player, int *Cards) { // not sure what it needs to return for display
    // 3-card poker
    int x = 0;
    CardSet cards[num_player];
    for(int round = 0; round < 3; ++round){  // First deal card1, then card2, then card3
        ...
        for(int player = 0; player < num_player; ++player){
            ...
            //scanf("%d", &x);
            if(round == 0){
                cards[player].card1 = Cards[x];
            }else if(round == 1){
                cards[player].card2 = Cards[x];
            }else{
                cards[player].card3 = Cards[x];
            }
            ++x;
        }
    }
